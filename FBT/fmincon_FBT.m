...
x_history = history.x;
f_history = history.fval;
save x.dat x_history -ascii;
save fval.dat f_history -ascii;
%===========================================================================================
...
%     end
    ...
    function [conie,cone] = pnonpcon(x)
        ...
        %py.**就表示调用python函数
        py.ANSYS_mac_update_FBT.ANSYSmacupdate(x(1),x(1),x(2),x(2));
        %--------------------By YJS------------------------------
        ...
        %system(crcommand);
        %+++++++++function++++++++
        py.Rm_tmp_file_FBT.RmTmpFile('F:/WorkPath/ANSYS/');
        %------------------By YJS------------------
    end
